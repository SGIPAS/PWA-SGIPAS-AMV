// ocp Operaciones CRUD – interacción con Supabase (tablas documentos, versiones, storage)
...
export async function cargarListaDocumentos(rol) {
        ...
        html += `
            <tr class="hover:bg-slate-700/50 transition duration-150">
                ...
                <td class="p-4 flex space-x-2">
                    ${doc.archivo_url ? `<a href="${supabase.storage.from('biblioteca').getPublicUrl(doc.archivo_url).data.publicUrl}" target="_blank" class="text-blue-400 hover:underline">Ver</a>` : '-'}
                    ${esAdmin ? `
    ...
    // ocp Funciones globales para los botones inline (solo admin)
...
export async function manejarSubmitDocumento(rol) {
    ...
    try {
        ...
        if (id) {
            // ocp Actualizar documento existente: crear nueva versión
            ...
            // ocp Insertar versión anterior en historial
            ...
            // ocp Registrar la nueva versión en historial
            await supabase.from('versiones').insert({
                documento_id: id,
            ...
            // ocp Nuevo documento
            if (!archivo_url) throw new Error('Debe seleccionar un archivo PDF');
            ...
            // ocp Primera versión en historial
            await supabase.from('versiones').insert({
