// ocp Submódulo de Fundición – big bags y acidez de tanques
import { supabase } from '../../supabase-client.js';

export async function renderizarFundicion(contenedor) {
            ...
            <div class="bg-slate-900 p-4 rounded border border-slate-700">
                ...
                <form id="form-fundicion" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div><label class="block text-slate-400 text-sm">Acidez TQ-A (%)</label><input type="number" step="0.01" id="fund-a" class="w-full bg-slate-800 border border-slate-700 rounded p-2 text-white"></div>
                        <div><label class="block text-slate-400 text-sm">Acidez TQ-B (%)</label><input type="number" step="0.01" id="fund-b" class="w-full bg-slate-800 border border-slate-700 rounded p-2 text-white"></div>
                        <div><label class="block text-slate-400 text-sm">Acidez TQ-C (%)</label><input type="number" step="0.01" id="fund-c" class="w-full bg-slate-800 border border-slate-700 rounded p-2 text-white"></div>
                        <div><label class="block text-slate-400 text-sm">Acidez TQ-D (%)</label><input type="number" step="0.01" id="fund-d" class="w-full bg-slate-800 border border-slate-700 rounded p-2 text-white"></div>
                    </div>
